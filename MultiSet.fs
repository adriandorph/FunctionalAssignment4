...
let toList (s:MultiSet<'a>) : List<'a> = fold (fun acc key value -> List.append acc [ for _ in 1u .. value -> key ]) List.Empty s

let map (f:('a -> 'b)) (set: MultiSet<'a>): MultiSet<'b> = fold (fun acc key value -> add (f key) value acc) empty set

let union (a:MultiSet<'a>) (b : MultiSet<'a>) : MultiSet<'a> = fold (fun acc key value -> add key (if value > (numItems key a) then value else (numItems key a)) acc) empty b

let sum (a:MultiSet<'a>) (b : MultiSet<'a>) : MultiSet<'a> = fold (fun acc key value -> add key (value + (numItems key a)) acc) empty b

let subtract (a:MultiSet<'a>) (b : MultiSet<'a>) : MultiSet<'a> = fold (fun acc key value -> remove key value acc) a b

let intersection (a:MultiSet<'a>) (b : MultiSet<'a>) : MultiSet<'a> = fold (fun acc key value -> add key (if value < (numItems key a) then value else (numItems key a)) acc) empty b
